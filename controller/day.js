...
const getDay = (req,res) => {
  ...
  pool
    .query(query)
    .then((dbRes) => { const sortedObj = Object.fromEntries(dbRes.rows.map((obj) => [obj.name, { id: obj.id, name: obj.name, spots: Number(obj.appointments) - Number(obj.interviews)}])); console.log(sortedObj); res.json(sortedObj); })
